                ...
                <form id="reservation_form">
                    ...
                    <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                        ...
                        <div class="sm:col-span-3">
                            ...
                            <div class="mt-2">
                                <input type="text" name="car-type" id="car-type" value="{{ $car->brand }} {{ $car->model }}"
                                       class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-pr-400 sm:text-sm sm:leading-6">
                            </div>
                            @error('car-type')
                            <span class="text-red-500">{{ $message }}</span>
                        ...
                        <div class="sm:col-span-full">
                            <label for="reservation_dates" class="block text-sm font-medium leading-6 text-gray-900">Reservation Dates</label>
                            <x-flatpickr range id="reservation_dates" name="reservation_dates" class="w-full rounded-md" placeholder="Reservation Dates"/>
                        </div>
                    </div>
                    <div class="mt-12 md:block hidden">
                        <button type="submit" class="text-white bg-pr-400 p-3 w-full rounded-lg font-bold hover:bg-black shadow-xl hover:shadow-none">
                            Order Now
                        </button>
                    </div>
                </form>
...
document.getElementById('reservation_form').addEventListener('submit', function(event) {
    ...
    var reservationDates = document.getElementById('reservation_dates').value;

    var message = `Full Name: ${fullName}\nEmail: ${email}\nPhone Number: ${phoneNumber}\nCar Type: ${carType}\nReservation Dates: ${reservationDates}`;

    var whatsappNumber = '201029094262'; // تأكد من إضافة رمز البلد للرقم
